package com.kts.catz.utils.plugin;
...
import com.kts.catz.utils.googlebilling.Purchase;
import com.kts.catz.utils.googlebilling.IabHelper;
import com.kts.catz.utils.googlebilling.IabResult;
import com.kts.catz.utils.googlebilling.Inventory;
import com.kts.catz.utils.googlebilling.SkuDetails;

import android.content.Intent;
import android.util.Log;

public class CatzUtilPlugin extends CordovaPlugin {
	private final Boolean ENABLE_DEBUG_LOGGING = true;
	private final String TAG = "CordovaCatzPlugin";
    ...
    public boolean execute(String action, JSONArray data, CallbackContext callbackContext) throws JSONException {
        ...
        if (action.equals("checkPlugin")) {			
			Log.d(TAG, "called plugin action - checkPlugin");

            String messageForPlugin = data.getString(0);
            String pluginResponse = "Response from plugin :\n";
			pluginResponse += "Platform : Android ;\n";
			pluginResponse += "Message passed from Android app : " + messageForPlugin + ";\n";
            callbackContext.success(pluginResponse);

			Log.d(TAG, "plugin action - checkPlugin - completed");
            return true;
        } 
		
		if ("initBillingModule".equals(action)) {
			Log.d(TAG, "called plugin action - initBillingModule");
			Log.d(TAG, "data.getString(0) - " + data.getString(0));
			...
			if ("billingGetPurchases".equals(action)) {
				Log.d(TAG, "called plugin action - billingGetPurchases");
			...
			if ("billingGetProductDetails".equals(action)) {
				Log.d(TAG, "called plugin action - billingGetProductDetails");
				JSONArray jsonSkuList = new JSONArray(data.getString(0));
				final List<String> sku = new ArrayList<String>();			
				int len = jsonSkuList.length();
				Log.d(TAG, "Num SKUs Found: "+len);
			...
			if ("billingPurchaseProduct".equals(action)) {
				Log.d(TAG, "called plugin action - billingPurchaseProduct");
				final String sku = data.getString(0);
				Log.d(TAG, "SKU for purchase : " + sku);
				itemToBuy = sku;
				purchaseProduct(sku);
				return true;
			}
			
			if ("consumeProduct".equals(action)) {
				Log.d(TAG, "called plugin action - consumeProduct");
	...
	// Buy an item
	private void purchaseProduct(final String sku){
	...
	private JSONArray getPurchases() throws JSONException {
		...
		if(myInventory == null){
			Log.d(TAG, IabHelper.ERR_REFRESH + "|Billing plugin was not initialized");
        ...
        public void onQueryInventoryFinished(IabResult result, Inventory inventory) {
            ...
            try {
                for (SkuDetails sku : skuList) {
					...
					Log.d(TAG, "SKUDetails: Description: "+sku.getDescription());
			...
			Log.d(TAG, "jsonSkuList : "+ jsonSkuList);
            callbackContext.success(jsonSkuList);
        }
    };
    ...
    IabHelper.QueryInventoryFinishedListener mGotInventoryListener = new IabHelper.QueryInventoryFinishedListener() {
        public void onQueryInventoryFinished(IabResult result, Inventory inventory) {
			...
			if(inventory.hasDetails(itemToBuy)){
		    	   Log.d(TAG, "Updating purchase item data - itemID=" + itemToBuy);
			}
    ...
    private Boolean hasErrorsAndUpdateInventory(IabResult result, Inventory inventory){
    	if (result.isFailure()) {
			Log.d(TAG, "Query inventory result failed. " + result.getResponse() + "|Failed to query inventory: " + result);
        ...
        // Have we been disposed of in the meantime? If so, quit.
        if (mHelper == null) {
			Log.d(TAG, "mHelper == null !!!! " + IabHelper.ERR_LOAD + "|The billing helper has been disposed");
